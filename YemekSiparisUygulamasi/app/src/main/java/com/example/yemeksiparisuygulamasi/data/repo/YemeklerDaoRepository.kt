...
import com.example.yemeksiparisuygulamasi.data.entity.*
...
class YemeklerDaoRepository(var ydao: YemeklerDao) {
    ...
    fun sepetYemekSil(sepet_yemek_id:Int,kullanici_adi:String){
        ydao.sepettenYemekSil(sepet_yemek_id,"FurkanUlengin").enqueue(object : Callback<CRUDCevap>{
            override fun onResponse(call: Call<CRUDCevap>?, response: Response<CRUDCevap>) {
                val basari = response.body()!!.success
                val mesaj = response.body()!!.message
                Log.e("Yemek sepete ekle", "$basari - $mesaj")
    ...
    fun tumSepetiGoster(){
        ydao.sepettekiYemekler("FurkanUlengin").enqueue(object:Callback<SepetCevap>{
